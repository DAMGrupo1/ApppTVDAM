...
SceneScene1.prototype.initialize = function () {
	...
	$.ajax({
		  ...
		  success: function(data){
		  	alert('success');
		  	//$("#movies").append('<div class="row"></div>');
		  	$("#movies").append('<div id="t'+max_index_number+'"><h1 class=titulo> SERIES POPULARES </h1></div>');
			//$("#movies").append('<div id="t'+max_index_number+'"><h1 class=titulo> PELICULAS MAS VALORADAS </h1></div></div>');
			 for (var i = 0; i < 6; i++) {
		  		if(i == 0) $("#movies").append('<div id="d'+max_index_number+'" class="row">');
		  		if(i == 5) $("#movies").append('</div>');
	...
	$.ajax({
		  ...
		  success: function(data){
		  	alert('success');
		  	//$("#movies").append('<div class="row"></div>');
		  	$("#movies").append('<div id="t'+max_index_number+'"></div><h1 class=titulo> PERSONAS POPULARES </h1></div>');
		  	for (var i = 0; i < 6; i++) {
		  		if(i == 0) $("#movies").append('<div id="d'+max_index_number+'" class="row">');
		  		if(i == 5) $("#movies").append('</div>');
		  		
		  		$("#movies").append('<div class="col-xs-2" tipo_url="Persona" href="'+data.results[i].id+'"><img src="'+base_url+'w342'+data.results[i].profile_path+'"/></div>');
		  	}	
	  		$("#movies").append('<div id="dd'+max_index_number+'" class="row"></div>');
		  	$('#movies div.col-xs-2').keynav();
		  	max_index_number++;
