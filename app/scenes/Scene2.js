...
SceneScene2.prototype.handleShow = function (data) {
	...
	if(tipo_url!='Serie' & tipo_url!='Persona'){
	$.ajax({
	  type: "GET",
	  crossDomain: true,
	  async: true,
	  ...
	  success: function(data){
		...
		$.ajax({
			  ...
			  async: true,
			  dataType: "json",
			  url: API+'/person/'+movie_id,
			  data: { api_key: api_key, language: 'es' },
			  success: function(data){
			  	$("#details").css('background-image', '');
			  	$("#cover").append('<img src="'+base_url+'w342'+data.profile_path+'"/>');
			  	$("#title").append('<h1>'+data.name+'</h1>');
			  	$("#title").append('<h2>'+data.birthday+' '+data.place_of_birth+'</h2>');
			  	$("#sinopsis").append(data.biography);	
			  								  }
			});	
	}
